#!/usr/bin/env python3
"""
Calculate Precision from curated experiment results.

This script:
1. Filters curated_data.jsonl based on criteria
2. Calculates precision for both default and improved Biomni results
"""
...
def load_pmids_from_json(json_path: str) -> Set[str]:
    """Load PMIDs from a JSON file (with deduplication)."""
    pmids = set()
...
def main():
    ...
    output_jsonl = base_dir / "results" / "curated_data" / "filtered_data.jsonl"
    ...
    default_Biomni_json = (
        ...
        / "results"
        / "Biomni_default"
        / "20251101_filtered_papers_database.json"
    )
    improved_Biomni_json = (
        base_dir
        / "results"
        / "Biomni_custom"
        / "20251031_filtered_papers_database.json"
    )
    ...
    # Load PMIDs from result files
    print("=" * 70)
    print("STEP 2: Loading PMIDs from result files")
    print("=" * 70)

    default_pmids = load_pmids_from_json(str(default_Biomni_json))
    improved_pmids = load_pmids_from_json(str(improved_Biomni_json))

    print(f"Default Biomni PMIDs loaded: {len(default_pmids)}")
    print(f"Improved Biomni PMIDs loaded: {len(improved_pmids)}")
    print()
    ...
    default_metrics = calculate_precision(default_pmids, correct_pmids)
    improved_metrics = calculate_precision(improved_pmids, correct_pmids)

    # Print results
    print("【Default Biomni】")
    print(f"  Total PMIDs: {default_metrics['total']}")
    print(f"  Correct: {default_metrics['correct']}")
    print(f"  Incorrect: {default_metrics['incorrect']}")
    print(f"  Precision: {default_metrics['precision']:.2f}%")
    print()

    print("【Improved Biomni】")
    print(f"  Total PMIDs: {improved_metrics['total']}")
    print(f"  Correct: {improved_metrics['correct']}")
    print(f"  Incorrect: {improved_metrics['incorrect']}")
    print(f"  Precision: {improved_metrics['precision']:.2f}%")
    print()

    # Calculate improvement
    if default_metrics["precision"] > 0:
        improvement = improved_metrics["precision"] - default_metrics["precision"]
        print("【Comparison】")
    ...
    print("【Default Biomni False Positives】")
    if default_metrics["false_positives"]:
        print(f"  Count: {len(default_metrics['false_positives'])}")
        print(f"  PMIDs: {', '.join(default_metrics['false_positives'])}")
    else:
        print("  None")
    print()

    print("【Improved Biomni False Positives】")
    if improved_metrics["false_positives"]:
        print(f"  Count: {len(improved_metrics['false_positives'])}")
        print(f"  PMIDs: {', '.join(improved_metrics['false_positives'])}")
    else:
        print("  None")
    print()

    # Save False Positives to files
    fp_dir = base_dir / "results" / "curated_data"

    default_fp_file = fp_dir / "false_positives_default.txt"
    with open(default_fp_file, "w", encoding="utf-8") as f:
        f.write("False Positives - Default Biomni\n")
        f.write("=" * 70 + "\n\n")
        f.write(f"Count: {len(default_metrics['false_positives'])}\n\n")
        f.write("PMIDs:\n")
        for pmid in default_metrics["false_positives"]:
            f.write(f"{pmid}\n")

    improved_fp_file = fp_dir / "false_positives_improved.txt"
    with open(improved_fp_file, "w", encoding="utf-8") as f:
        f.write("False Positives - Improved Biomni\n")
        f.write("=" * 70 + "\n\n")
        f.write(f"Count: {len(improved_metrics['false_positives'])}\n\n")
        f.write("PMIDs:\n")
        for pmid in improved_metrics["false_positives"]:
            f.write(f"{pmid}\n")

    print("=" * 70)
    print(f"Filtered data saved to: {output_jsonl}")
    print(f"Default FP saved to: {default_fp_file}")
    print(f"Improved FP saved to: {improved_fp_file}")
    print("=" * 70)
